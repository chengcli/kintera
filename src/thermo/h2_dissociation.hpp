...
// species.
//
// WHY: a host code that asks kintera every thermodynamic question ("VT->P",
// "VU->T", "VT->cv", eval_czh, ...) can then represent a dissociating gas
// WITHOUT advecting H: kintera resolves the equilibrium internally at (T, c)
// and reports the resulting particle count, internal energy and heat
// capacities. No extra tracers, no chemistry operator, no stiff solve.
//
// The lumped species has composition {H: nH, He: nHe} per mole. At (T, c):
//
//   ln Kp = -(2 g_H - g_H2)        (g = G/RT from the NASA-9 fits)
//   Kc    = Kp * P0 / (R T)
//   [H]   = (-Kc + sqrt(Kc^2 + 8 Kc nH c)) / 4    (one closed-form quadratic)
//   [H2]  = (nH c - [H]) / 2,   [He] = nHe c
//
// and then, ALL DERIVED FROM THAT SAME SPECIATION so they are mutually
// consistent:
//
//   cz   = n_tot / c                      particles per mole (>1 once
//                                         dissociated)
//   e/R  = sum_i n_i (h_i/R - T) / c      NASA-9 h is ABSOLUTE, so the
//                                         436 kJ/mol bond energy lands here
//   cv/R = [sum_i n_i cv_i/R + sum_i (u_i/R) dn_i/dT] / c
//                                         the 2nd term IS the latent heat
//   cp/R = cv/R + (cz + T dcz/dT)^2 / (cz + c dcz/dc)
//                                         exact reacting-gas Mayer relation
//
// Consistency is the whole point: grad_ad is EMERGENT from (P, u, cv), not
// imposed. Correcting cp alone is NOT enough: the particle-number term is a
// sizeable part of grad_ad in a dissociating gas.
...
// MUST match kNasa9Tref in eval_uhs.cpp (energy continuity)
