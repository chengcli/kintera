...
def _th(tmp_path, max_iter=None):
    ...
    # uv-solver kkt: these disjoint pairs would otherwise take the partition path, which has no gain
    eos = "uv-solver: kkt" if max_iter is None else "uv-solver: kkt, max-iter: %d" % max_iter
    p.write_text(YAML + "dynamics: {equation-of-state: {%s}}\n" % eos)
