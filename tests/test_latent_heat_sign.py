"""A condensate whose internal energy exceeds its vapour's must be reported at load.
...
absorbs energy -- and ThermoOptions.from_yaml warns (TORCH_WARN, i.e. on stderr).
...
def _load(tmp_path, u0):
    """Load one card in a fresh interpreter; return what it wrote to stderr."""
...
def test_endothermic_condensate_is_reported(tmp_path):
    err = _load(tmp_path, None)
    assert "Warning: reaction 'vapor => cloud' has latent heat 0 K <= 0" in err, err
