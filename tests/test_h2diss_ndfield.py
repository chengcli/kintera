...
If speciate() passed T as (spatial...,1) to helpers that reduce the coeff axis via select
-> (spatial...), it would OUTER-PRODUCT T against the field: invisible at n=1, wrong for any
block of more than one cell (e.g. a cons2prim over a whole mesh block)."""
import numpy as np
import pytest
import torch
...
def test_cons2prim_roundtrip_on_block(th):
    ...
    T_vu = th.compute("VU->T", (V, U))           # the inverse a host code uses in cons2prim
