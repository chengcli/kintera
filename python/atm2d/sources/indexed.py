"""Built-in cell-local source-term shapes used by atm2d-driven networks.

Each class implements the :class:`LocalSourceTerm` protocol by returning
a :class:`LocalSourceLinearization` with both tendency and analytic
Jacobian. Concrete shapes:

  - :class:`IndexedFirstOrderSource`         — A → products (rate × [A])
  - :class:`IndexedMassActionSource`         — multi-reactant mass action
  - :class:`IndexedReversibleFirstOrderSource` — A ⇌ B exchange
  - :class:`IndexedBoundaryFluxSource`       — constant flux at one boundary
  - :class:`IndexedBoundaryVelocitySource`   — velocity flux at one boundary
"""
...
from .protocol import LocalSourceLinearization, RateProvider
