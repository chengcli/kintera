...
def parse_kb_prodloss(path: pathlib.Path) -> tuple[list[int], np.ndarray, np.ndarray]:
    ...
    if path.name.endswith("_loss.dat") and "kb_run_500" in str(path):
        # Legacy compensation for the May-2026 KB run (kb_run_500/),
        # which was generated by the buggy KB binary that wrote
        # srate(_, 1, 2, kk). For runs produced by the patched KB
        # (diagnostics/kb_patches/01-loss-file-altitude-shift.patch),
        # the file is already correct; no shift needed. Gate on the
        # directory name to avoid double-correcting.
